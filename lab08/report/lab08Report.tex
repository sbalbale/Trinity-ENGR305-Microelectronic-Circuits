...
\author{Sean Balbale}
\date{\today} % Or specify: {October 29, 2025} % Updated date
...
$$
% R1, R2 calculations updated to use Thevenin method
...
% Note: Simulation results might change slightly with the new R1, R2. Assuming previous values are representative for now.
\begin{itemize}
...
\begin{itemize}
    ...
    \item $R_1 = \SI{141.45}{\kilo\ohm}$, $R_2 = \SI{157.03}{\kilo\ohm}$, $R_C = \SI{10.02}{\kilo\ohm}$, $R_E = \SI{14.7}{\kilo\ohm}$ % Note: Measured R1/R2 are closer to the old calculation.
...
\begin{itemize}
    \item $V_+ = \SI{15.01}{\volt}$, $V_- = \SI{-15.02}{\volt}$
    \item $R_1 = \SI{8.6356}{\kilo\ohm}$, $R_2 = \SI{6.652}{\kilo\ohm}$, $R_C = \SI{12.945}{\kilo\ohm}$, $R_E = \SI{14.7}{\kilo\ohm}$ % Note: Measured R1/R2 are closer to the old calculation.
...
\begin{itemize}
  ...
  \item \textbf{Transistor Model:} $\beta = 100$ 
  \item \textbf{Circuit:} Diode-connected ($V_B = V_C$)  % Implied by description
  \item \textbf{Assumption:} Active region $V_{EB(\text{on})} = \SI{0.7}{\volt}$ 
\end{itemize}
